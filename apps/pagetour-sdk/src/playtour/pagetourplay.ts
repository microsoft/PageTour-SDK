...
class PageTourPlay {
  ...
  public runTour = (
    ...
    if (
      objTour.coverPage &&
      objTour.coverPage.location.toLowerCase() === "start"
    ) {
      this.showCoverPageModal(
        objTour,
        action,
        callback,
        startInterval,
      ...
      this.beginTourSteps(
        ...
        startInterval,
        autoPlayTest
      )(objTour, action, callback, startInterval, autoPlayTest);
    }
  };
  ...
  public runSmartTip = (
    ...
    objTour.steps.forEach((element, i) => {
      setTimeout(
        () => this.makeSmartTipVisible(element, i, objTour, callback),
        element.delayBefore * 1000
      );
    });
  ...
  private makeSmartTipVisible(
    ...
    if (!smartTipElement) {
      if (selectedElement && !selectedElement.getAttribute("disabled")) {
        ...
        (
          smartTipPopup.getElementsByClassName(
            "smarttip-content"
          )[0] as HTMLParagraphElement
        ).innerText = element.message;
        (
          smartTipPopup.getElementsByClassName(
            "smarttip-dismiss"
          )[0] as HTMLButtonElement
        ).addEventListener("click", () => {
          this.dismissSmartTip(
            smartTipId,
            objTour,
            "Completed",
            (objTour.steps.length - 1).toString(),
            "Dismissed"
          );
          if (callback != null) callback(objTour.tourtype);
        });
        (
          smartTipPopup.getElementsByClassName(
            "smarttip-dismiss"
          ...
          if (event.key === "Enter") {
            let smartTipFocasibleElement = document
              .getElementById(smartTipId)
              .getElementsByClassName("smart-tip")[0] as HTMLElement;
            let focusableElementsOutSide = document.body.querySelectorAll(
              'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
            );
            Array.from(focusableElementsOutSide).forEach(
              (element, indexOfElemet) => {
                if (element === smartTipFocasibleElement) {
                  if (focusableElementsOutSide.length > indexOfElemet + 1) {
                    (
                      focusableElementsOutSide[indexOfElemet + 1] as HTMLElement
                    ).focus();
                  } else {
        ...
        (
          ...
          smartTipPopup.style.display = "none";
          if (callback != null) callback(objTour.tourtype);
        });
        (
          smartTipPopup.getElementsByClassName(
        ...
        smartTip.addEventListener("mouseover", function () {
          objThis.setToolTipPopupArrowPointer(
            objTour,
            element,
            arrowDiv,
            smartTipPopup,
        ...
        smartTip.addEventListener("keydown", function (event) {
          ...
          if (event.key === "Enter") {
            ...
            let focusableElements = smartTipPopup.querySelectorAll(
              'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
            );
            ...
            let availableSmartPopUps =
              document.getElementsByClassName("smarttip-container");

            for (let i = 0; i < availableSmartPopUps.length; i++) {
              let currPopupInst = availableSmartPopUps[i] as HTMLElement;
              if (
                currPopupInst.id === smartTipPopup.id &&
                currPopupInst.style.display != "none"
              ) {
                (
                  currPopupInst.getElementsByClassName(
                    "smarttip-close"
                  )[0] as HTMLElement
                ).focus();
                break;
        ...
        let smartTipFocusObj = smartTip.getElementsByClassName(
          "smart-tip"
        )[0] as HTMLElement;

        smartTipFocusObj.addEventListener("focusin", function (event) {
          smartTipFocusObj.setAttribute("role", "application");
          smartTipFocusObj.setAttribute(
            "aria-label",
            "Smart tip icon for " + objTour.title
          );
        });
        ...
        if (opts.navigator.callbackOnTourStepFailure != null) {
          ...
          opts.navigator.callbackOnTourStepFailure(
            objTour,
            0,
            stepErrorMessage
          );
  ...
  private setToolTipPopupArrowPointer(
    objTour: Tutorial,
    element: any,
    arrowDiv: HTMLDivElement,
    smartTipPopup: HTMLElement,
    smartTip: HTMLElement,
    i: number,
    zIndex: string
  ) {
    Array.from(
    ...
    let toolTipPopper = document.getElementById(
      `smarttip_${objTour.id}_${i}-popup`
    );
  ...
  public runAnnouncement = (
    ...
    this.LaunchAnnouncement(
      ...
      callback,
      autoPlayTest
    )(objTour, action, startInterval, callback, autoPlayTest);
  };
    ...
    return function (
      ...
      if (autoPlayTest === true) {
        ...
        setTimeout(() => {
          ...
          self.executeAnnouncementNextStep(
            tour,
            action,
            0,
            0,
          ...
          const nextButton: HTMLButtonElement =
            document.querySelector("#anno-next-step");
          if (nextButton) {
            nextButton.addEventListener("click", () => {
              self.goToAnnouncementNextStep(
                StepAction.Next,
                tour,
                action,
                callback,
          ...
          const previousButton: HTMLButtonElement = document.querySelector(
            "#anno-previous-step"
          );
          if (previousButton) {
          ...
          const exitButton: HTMLButtonElement =
            document.querySelector("#anno-exit-step");
          if (exitButton) {
          ...
          const audioMuteButton: HTMLButtonElement = document.querySelector(
            "#announcement-audio"
          );
          if (audioMuteButton) {
            audioMuteButton.addEventListener("click", () => {
              document.getElementById("announcement-audio-stop").style.display =
                "inline";
              document.getElementById("announcement-audio").style.display =
                "none";
              self.isMuted = true;
          ...
          const audioUnMuteButton: HTMLButtonElement = document.querySelector(
            "#announcement-audio-stop"
          );
          if (audioUnMuteButton) {
            audioUnMuteButton.addEventListener("click", () => {
              ...
              if (opts.navigator.callbackOnVolumeUnmute != null) {
                opts.navigator.callbackOnVolumeUnmute(
                  self.tour.steps[self.currentStep].transcript
                );
              }
            });
          }
  ...
  private beginTourSteps = (
    ...
    return function (
      ...
      if (autoPlayTest === true) {
        ...
        let tourEndsWithCoverPage =
          tour.coverPage && tour.coverPage.location.toLowerCase() === "end";
        ...
        setTimeout(() => {
          ...
          const exitButton: HTMLButtonElement = document.querySelector(
            "#pagetour-exit-step"
          );
          if (exitButton) {
            exitButton.addEventListener("click", () => {
          ...
          const audioMuteButton: HTMLButtonElement =
            document.querySelector("#pagetour-audio");
          if (audioMuteButton) {
            audioMuteButton.addEventListener("click", () => {
              document.getElementById("pagetour-audio-stop").style.display =
                "inline";
          ...
          if (audioUnMuteButton) {
            audioUnMuteButton.addEventListener("click", () => {
              document.getElementById("pagetour-audio-stop").style.display =
                "none";
              document.getElementById("pagetour-audio").style.display =
                "inline";
              self.isMuted = false;
              if (opts.navigator.callbackOnVolumeUnmute != null) {
                opts.navigator.callbackOnVolumeUnmute(
  ...
  private goToNextStep = (
    ...
    if (stepAction === StepAction.Next) {
      ...
      let element = querySelectorDeep(
        self.getElementSelector(self.currentStep)
      );
      ...
      if (self.currentStep === self.totalSteps - 1) {
        ...
        if (tourEndsWithCoverPage) {
          ...
          if (
            opts.feedback.PagetourFeedbackOptions &&
            opts.feedback.PagetourFeedbackOptions.enabled &&
            action != RunTourAction.Preview
          ) {
    ...
    if (stepAction === StepAction.Next) {
      ...
      if (self.currentStep === self.totalSteps - 1) {
        ...
        if (self.currentStep === this.totalSteps - 1) {
          ...
          self.cleanupAction(element);
          if (opts.navigator.callbackAfterTourEnd != null) {
            opts.navigator.callbackAfterTourEnd(
              self.tour,
              self.currentStep + 1
            );
          }
  ...
  private initializeAnnouncement = (tour: any, action: RunTourAction) => {
    ...
    this.currentStep = 0;
    this.isMuted = false;
    let annoFeedbackOpts =
      this.configStore.Options.feedback.AnnouncementFeedbackOptions;
    if (
      annoFeedbackOpts &&
      annoFeedbackOpts.enabled == true &&
      action != RunTourAction.Preview
    ) {
      var jsonFeedbackObj: AnnouncementFeedbackStep[] = [];
      var ratingStep: AnnouncementFeedbackStep = {
        submitted: false,
        rating: 0.0,
        ratingElement: null,
      };
  ...
  private navigateToStart = (pageContext: PageContext) => {
    ...
    if (opts.navigator && opts.navigator.navigateToContext) {
      ...
      window.location.href =
        window.location.protocol + "//" + window.location.host + pageContext;
    }
  };
  ...
  private setupAnnouncementBox = (tour: any, action: RunTourAction) => {
    ...
    if (
      annoFeedbackOpts &&
      annoFeedbackOpts.enabled &&
      action != RunTourAction.Preview
    ) {
      let announcementFeedbackDivElement = document.getElementById(
        "feedbackelement"
      ) as HTMLElement;
      announcementFeedbackDivElement = DomUtils.appendTo(
        announcementFeedbackDivElement,
        this.announcementFeedbackPageFn()
      );
    } else {
  ...
  private executeNextStep = (
    ...
    if (currentStep.ignoreStepIf && currentStep.ignoreStepIf === true) {
      if (
        currentStep.ignoreStepIfConditions &&
        currentStep.ignoreStepIfConditions === "NextStepElementFound"
      ) {
    ...
    if (this.isValidElement(element)) {
      const previoustButton: HTMLButtonElement = document.querySelector(
        "#pagetour-previous-step"
      );
      const nextButton: HTMLButtonElement = document.querySelector(
        "#pagetour-next-step"
      );
      const audioButton: HTMLButtonElement =
        document.querySelector("#pagetour-audio");
      const audioMuteButton: HTMLButtonElement = document.querySelector(
        "#pagetour-audio-stop"
      );
      ...
      if (
        tour.steps[stepCount].transcript &&
        tour.steps[stepCount].transcript !== ""
      ) {
        if (this.isMuted) {
      ...
      if (stepCount === 0) {
        ...
        if (
          action === RunTourAction.Play &&
          (!opts.isCoverPageTourStart ||
            !tour.coverPage ||
            tour.coverPage == null ||
            tour.coverPage.location.toLowerCase() != "start")
        ) {
      ...
      if (!ignoreCurrentStep) {
        ...
        let tourBoxElement: HTMLElement =
          document.getElementById("pagetour-tourBox");
        DomUtils.manageTabbingWithoutFocus(tourBoxElement);
        if (opts.navigator.callbackAfterTourStep != null) {
          opts.navigator.callbackAfterTourStep(
            this.tour.steps[stepCount],
            this.isMuted
          );
        }
      }
  ...
  private executeAnnouncementNextStep = (
    ...
    const nextButton: HTMLButtonElement =
      document.querySelector("#anno-next-step");
    const annoCounter: HTMLButtonElement =
      document.querySelector("#annoboxcounter");
    const audioButton: HTMLButtonElement = document.querySelector(
      "#announcement-audio"
    );
    const audioMuteButton: HTMLButtonElement = document.querySelector(
      "#announcement-audio-stop"
    ...
    if (
      this.tour.steps[stepCount].mediaUrl !== undefined &&
      this.tour.steps[stepCount].mediaUrl !== ""
    ) {
    ...
    let defaultAnnoFeedbackOpts =
      this.configStore.DefaultOptions.feedback.AnnouncementFeedbackOptions;
    ...
    if (
      ...
      let annoFeedbackType =
        annoFeedbackOpts.type === undefined
          ? defaultAnnoFeedbackOpts.type
          : annoFeedbackOpts.type;
      ...
      if (this.announcementFeedbackObj[this.currentStep].submitted == false) {
        ...
        if (annoFeedbackType == AnnouncementFeedbackEnum.LikeRating) {
          let likeratingRadioButtons = document.querySelector(
            'input[name="likerating"]:checked'
          ) as HTMLInputElement;
          if (likeratingRadioButtons) {
            likeratingRadioButtons.checked = false;
          }
          ratingElement = document.getElementsByName(
            "likerating"
          ) as NodeListOf<HTMLInputElement>;
        } else if (annoFeedbackType == AnnouncementFeedbackEnum.YesNoRating) {
          let yesratingRadio = document.querySelector(
            'input[name="yesnorating"]:checked'
          ) as HTMLInputElement;
          if (yesratingRadio) {
        ...
        for (var i = 0; i < ratingElement.length; i++) {
          ratingElement[i].addEventListener("click", function (e) {
            if (
              this.checked &&
              self.announcementFeedbackObj[self.currentStep].submitted == false
            ) {
              ...
              self.announcementFeedbackObj[self.currentStep].ratingElement =
                this;
              if (opts.navigator.callbackOnFeedbackSubmit != null) {
                opts.navigator.callbackOnFeedbackSubmit(
                  rating,
                  self.tour,
                  self.currentStep
                );
              }
              let feedbackContentElement =
                document.getElementById("feedbackContent");
              let privacyElement = document.getElementById(
                "annoFeedbackPrivacy"
              );
              let submitMessageElement =
                document.getElementById("annoThanksMsg");
        ...
        this.announcementFeedbackObj[this.currentStep].submitted == true
      ) {
  ...
  private ApplyTheme(stepCount: number) {
    ...
    if (this.tourTheme.isRounded) {
      tourboxdata.style.borderRadius = this.tourTheme.borderRadius
        ? `${this.tourTheme.borderRadius}px`
        : "10px";
    } else {
    ...
    tourboxdata.style.fontFamily = this.tourTheme.fontFamily
      ? this.tourTheme.fontFamily
  ...
  private getBorderWidthCSSString(
    top: number,
    right: number,
    bottom: number,
    left: number
  ...
  private cleanupAction = (element: HTMLElement) => {
    ...
    let elementOnRemovedListener =
      this.datastore["pagetour_nodeRemovedListener"];
    if (elementOnRemovedListener) {
      elementOnRemovedListener.removeEventListener(
        "DOMNodeRemoved",
        this.elementDomRemoved
      );
  ...
  private addTourOutline = (element: HTMLElement, tourType: string) => {
    if (element && !element.getAttribute("disabled")) {
      if (
        tourType.toLocaleLowerCase() ===
        TourTypeEnum.InteractiveGuide.toLowerCase()
      ) {
        this.datastore["pagetour_lastoutline"] = element.style.outline;
        //element.className += " tutorial-bubble";
        element.style.outline =
          this.configStore.Options.theme.primaryColor + " solid 5px";
        ...
        let height = element.offsetHeight;
        var rect = element.getBoundingClientRect();
        let pagetourHelperLayer = document.getElementById(
          "pagetour-elementLayer"
        );
        ...
        document.getElementById("pagetour-elementLayer").style.display =
          "inline";
      }
    }
  };
  ...
  private elementDomRemoved = (event: any) => {
    if (event) {
      ...
      if (elementRemoved) {
        if (elementOnRemovedListener) {
          elementRemoved.removeEventListener(
            "DOMNodeRemoved",
            this.elementDomRemoved
          );
          this.datastore["pagetour_nodeRemovedListener"] = null;
  ...
  private updateUserActions = async (
    tour: Tutorial,
    userAction: string,
    step: string,
    operation: string
    ...
    try {
      let response =
        await this.configStore.Options.userActionProvider.recordUserAction(
          tour,
          userAction,
          step,
          operation
  ...
  private showCoverPageModal = (
    ...
    if (!this.modal) {
      ...
      let coverPageContentElement = document.getElementById(
    ...
    this.modal.style.fontFamily = this.tourTheme.fontFamily
      ? this.tourTheme.fontFamily
      : this.defaultFontFamily;
    this.modal.style.display = "block";
    let startTourCoverPageButton = document.getElementById(
      "starttour-cover-page-btn"
    ) as HTMLButtonElement;
    let closeBtn = document.getElementById(
      "cover-page-close-btn"
    ) as HTMLButtonElement;
    ...
    if (this.tourTheme.isRounded) {
      coverpagebox.style.borderRadius = this.tourTheme.borderRadius
        ? `${this.tourTheme.borderRadius}px`
        : "10px";
    }
    ...
    if (locationValue.toLowerCase() === "start") {
      ...
      startTourCoverPageButton.onclick = (event: MouseEvent) => {
        this.beginTourSteps(this.tour, action, callback, startInterval)(
          this.tour,
          action,
          callback,
          startInterval
      ...
      startTourCoverPageButton.onclick = this.closeCoverPageModal(
        callback,
        action
      );
    }
      ...
      if (locationValue.toLowerCase() === "start") {
        setTimeout(() => {
          self.beginTourSteps(
            ...
            startInterval,
            autoplaytest
          )(self.tour, action, callback, startInterval, autoplaytest);
        }, startInterval);
      } else {
        setTimeout(() => {
  ...
  private closeCoverPageModal = (callback: any, action: RunTourAction) => {
    return () => {
      ...
      if (
        opts.feedback.PagetourFeedbackOptions &&
        opts.feedback.PagetourFeedbackOptions.enabled &&
        coverPageLocation.toLocaleLowerCase() == "end" &&
        action != RunTourAction.Preview
      ) {
  ...
  private showFeedbackModal = () => {
    ...
    let opts = this.configStore.Options;
    let pagetourFeedbackOpts =
      this.configStore.Options.feedback.PagetourFeedbackOptions;
    let defaultFeedbackOpts =
      this.configStore.DefaultOptions.feedback.PagetourFeedbackOptions;

    let feedbackHeadingElement = document.getElementById("feedbackHeading");
    let feedbackDescriptionElement = document.getElementById(
      "feedbackDescription"
    );

    let feedbackHeading =
      pagetourFeedbackOpts.heading === undefined
        ? defaultFeedbackOpts.heading
        : pagetourFeedbackOpts.heading;
    let feedbackDescription =
      pagetourFeedbackOpts.description === undefined
        ? defaultFeedbackOpts.description
        : pagetourFeedbackOpts.description;

    feedbackHeadingElement.innerHTML = feedbackHeading;
    feedbackDescriptionElement.innerHTML = feedbackDescription;

    let feedbackType =
      pagetourFeedbackOpts.type === undefined
        ? defaultFeedbackOpts.type
        : pagetourFeedbackOpts.type;
    ...
    if (feedbackType.toLocaleLowerCase() == PagetourFeedbackEnum.StarRating) {
      ...
      feedbackType.toLocaleLowerCase() == PagetourFeedbackEnum.LikeRating
    ) {
      feedback5starElement.style.display = "none";
    }

    let feedbackPrivacyDescriptionElement =
    ...
    if (
      feedbackPrivacyDescription !== undefined &&
      feedbackPrivacyURL !== undefined
    ) {
    ...
    iconColor.style.setProperty(
      "--like-dislike-onclick-color",
      this.tourTheme.primaryColor
    );
    feedbackBoxDataElement.style.fontFamily = this.tourTheme.fontFamily
      ? this.tourTheme.fontFamily
      : this.defaultFontFamily;
    ...
    if (feedbackType.toLocaleLowerCase() == PagetourFeedbackEnum.StarRating) {
      ratingElementGroup = document.getElementsByName(
        "5starrating"
      ) as NodeListOf<HTMLInputElement>;
    } else if (
      feedbackType.toLocaleLowerCase() == PagetourFeedbackEnum.LikeRating
  ...
  private updateAnnouncementFeedbackElement = () => {
    ...
    let defaultAnnoFeedbackOpts =
      this.configStore.DefaultOptions.feedback.AnnouncementFeedbackOptions;
    let annoFeedbackHeadingElement = document.getElementById(
      "annoFeedbackHeading"
    );
    let annoPrivacyURLElement = document.getElementById("annoPrivacyUrl");
    let annoPrivacyDescElement = document.getElementById(
      "annoPrivacyDescription"
    );
    let submitMessageElement = document.getElementById("annoThanksMsg");
    let privacyElement = document.getElementById("annoFeedbackPrivacy");

    let annoFeedbackHeading =
      annoFeedbackOpts.heading === undefined
        ? defaultAnnoFeedbackOpts.heading
        : annoFeedbackOpts.heading;
    let annoPrivacyURL = annoFeedbackOpts.privacyURL;
    let annoPrivacyDesc = annoFeedbackOpts.privacyDescription;
    let annoFeedbackSubmitMsg =
      annoFeedbackOpts.submitMessage === undefined
        ? defaultAnnoFeedbackOpts.submitMessage
        : annoFeedbackOpts.submitMessage;
