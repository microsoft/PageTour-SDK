...
class PageTourManager {
  ...
  public autoPlayByContext = async (
    context: PageContext,
    startInterval: number,
    autoPlayViewedTour: boolean
  ) => {
  ...
  private openTourMangerDialog = async () => {
    ...
    this.isAuthorizedToAddTour =
      await this.authorizationService.isAuthorizedToAddTour();
    let isExportEnabled = await this.isExportAvaialable();
    this.isAuthorizedToDeleteTour =
      await this.authorizationService.isAuthorizedToDeleteTour();
    let isBeaconEnabled = this.configStore.Options.enableBeacon;
    let isAnnouncementEnabled = this.configStore.Options.enableAnnouncement;

    if (!this.isAuthorizedToAddTour) {
      document.getElementById("manage-tours-modal-add-tour-btn").style.display =
        "none";
    ...
    if(!isAnnouncementEnabled){
      document.getElementById("option-system-announcement").style.display = "none";
    }
    ...
    switch (result) {
      ...
      case this.ACTION_SUCCESS:
        if (htmlElement) {
          ...
          let exportButton = document.getElementById(
            `button-export_${id}`
          ) as HTMLButtonElement;
  ...
  private applyTheme(popupModalBox: any, popupOption1Button: any) {
    popupModalBox.style.fontFamily = this.tourTheme.fontFamily
      ? this.tourTheme.fontFamily
      : this.defaultFontFamily;
    popupModalBox.style.borderColor = this.tourTheme.primaryColor;

    if (this.tourTheme.isRounded) {
      popupModalBox.style.borderRadius = this.tourTheme.borderRadius
  ...
  private closeManageToursModal = () => {
    let manageTourModal = document.getElementById("manage-tours-modal");
    if (manageTourModal != null || manageTourModal != undefined)
      manageTourModal.parentNode.removeChild(manageTourModal);
  };
  ...
  /// Searches the Tours
  private searchTours = () => {
    const searchByTitleInputElement: HTMLInputElement = document.getElementById(
      "searchbytitle"
    ) as HTMLInputElement;
    const searchTourCancelButton: HTMLButtonElement = document.getElementById(
      "search-tour-btn-cancel"
    ) as HTMLButtonElement;
    const currentToursPageTop: HTMLInputElement = document.getElementById(
      "current-tours-page-top"
    ) as HTMLInputElement;
  ...
  private sortTours = () => {
    let dateTimeClass = document
      .getElementById("icon-tours-sortorder")
      .getAttribute("class");
    ...
    document
      .getElementById("icon-tours-sortorder")
      .setAttribute("class", dateTimeClass);
  ...
  private movetoToursNextPage = () => {
    let currentToursPageTop: HTMLInputElement = document.getElementById(
      "current-tours-page-top"
    ) as HTMLInputElement;
    ...
    currentToursPageBottom.valueAsNumber =
      parseInt(currentToursPageBottom.value, 10) + 1;
    this.populateTours();
  };
  ...
  private movetoToursPreviousPage = () => {
    ...
    currentToursPageTop.valueAsNumber =
      parseInt(currentToursPageTop.value, 10) - 1;
    currentToursPageBottom.valueAsNumber =
      parseInt(currentToursPageBottom.value, 10) - 1;
    this.populateTours();
  };
  ...
  private populateTours = () => {
    ...
    if (searchText.length > 1) {
      ...
      tours = tours.filter(function (tour: any) {
        if (
          ...
          (tour.lastmodifiedon &&
            tour.lastmodifiedon.toLowerCase().includes(searchText)) ||
          (tour.pagecontext &&
            tour.pagecontext.some(
              (x: string) =>
                x && x !== null && x.toLowerCase().includes(searchText)
            ))
        ) {
          return true;
        } else {
    ...
    let itemstoDisplay = parseInt(
      itemstoDisplayElement.options[itemstoDisplayElement.selectedIndex].value,
      10
    );

    const totalToursPagesTop: HTMLInputElement = document.getElementById(
      "total-tours-pages-top"
    ) as HTMLInputElement;
    ...
    const toursTablePreviousPageTop: HTMLButtonElement =
      document.getElementById(
        "tours-table-previous-page-top"
      ) as HTMLButtonElement;
    const toursTablePreviousPageBottom: HTMLButtonElement =
      document.getElementById(
        "tours-table-previous-page-bottom"
      ) as HTMLButtonElement;
    ...
    do {
      ...
      if (author != null) tdTitle.appendChild(author);
      ...
      startIndex < currentPage * itemstoDisplay &&
      startIndex < tours.length
    );
  };
    ...
    switch (property) {
      ...
      case "lastmodifiedby":
        msgElement.appendChild(
          document.createTextNode("Author: " + tour.lastmodifiedby)
        );
  ...
  private exportTour = async (id: any) => {
    let exportButtonElement: HTMLButtonElement = document.getElementById(
      "icon-export_" + id
    ) as HTMLButtonElement;
    let parentCell: HTMLTableCellElement =
      exportButtonElement.parentElement as HTMLTableCellElement;
    ...
    announcementElement.setAttribute(
      "aria-label",
      `Exporting tour ${tourToExport.title}`
    );
    ...
    await this.pagetourAuthor
      ...
      .then(() => {
        announcementElement.setAttribute(
          "aria-label",
          `Exported tour ${tourToExport.title}`
        );
        this.populateTours();
        if (this.configStore.Options.navigator.callbackOnTourExported) {
          this.configStore.Options.navigator.callbackOnTourExported(
            tourToExport
          );
        }
      })
      .catch((error) => {
        ...
        announcementElement.setAttribute(
          "aria-label",
          `Unable to export tour ${tourToExport.title}`
        );
  ...
  private toggleTour = (tourId: string) => {
    ...
    document
      .getElementById("tour-title_" + tourId)
      .setAttribute("class", messageClass);
    document
      .getElementById("tour-description_" + tourId)
      .setAttribute("class", messageClass);
    document
      .getElementById("tour-startpageurl_" + tourId)
      .setAttribute("class", messageClass);
  };
  ...
  private openExportPopup = async (tourId: any) => {
    await this.openPopup(
      tourId,
      "Export",
      "Do you want to export this tour?",
      "Export",
      "Cancel"
    );
  };
  ...
  private deleteTour = async (tourId: any) => {
    ...
    announcementElement.setAttribute(
      "aria-label",
      `Deleting tour ${tour.title}`
    );

    let deleteButtonElement: HTMLButtonElement = document.querySelectorAll(
      ".button-delete"
    )[index] as HTMLButtonElement;
    let parentCell: HTMLTableCellElement =
      deleteButtonElement.parentElement as HTMLTableCellElement;
  ...
  private changeButtonToSpinner = (
    ...
    spinnerDiv.classList.add(
      "pagetour-spinner-border",
      "pagetour-spinner-border-sm"
    );
    parentCell.appendChild(spinnerDiv);
  };

  private revertSpinnerToButton = (
    buttonElement: HTMLButtonElement,
    parentCell: HTMLTableCellElement
  ) => {
  ...
  /// Plays Tour
  private playTour = async (
    tourId: any,
    startInterval: number,
    autoPlayTest: boolean = false
  ) => {
    this.hideManagePageTourModal();
    await this.pageTourPlay.playTour(
      tourId,
      RunTourAction.Play,
      startInterval,
      autoPlayTest
  ...
  private playTourByObject = async (tour: any, startInterval: number) => {
    ...
    else this.pageTourPlay.runTour(tour, RunTourAction.Play, startInterval);
  };
  ...
  private isExportAvaialable = async (): Promise<Boolean> => {
    if (
      this.isExportEnabled == undefined ||
      this.isExportEnabled == null ||
      this.isExportEnabled == false
    ) {
      this.isAuthorizedToExportTour =
        await this.authorizationService.isAuthorizedToExportTour();
