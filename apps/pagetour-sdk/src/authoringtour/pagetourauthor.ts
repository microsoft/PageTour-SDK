...
class PageTourAuthor {
  ...
  public EditTour = (objTour: any) => {
    ...
    document
      .getElementById("add-tour-modal-cancel-btn")
      .setAttribute("aria-label", "cancel and close Edit Tour dialog");
  ...
  private addTourDialog = (tourType: string) => {
    ...
    (document.getElementById("tour-activeon") as HTMLInputElement).value =
      new Date().toISOString().split("T")[0];
    let defaultExpiresOn = new Date().setMonth(new Date().getMonth() + 6);
    (document.getElementById("tour-expireson") as HTMLInputElement).value =
      new Date(defaultExpiresOn).toISOString().split("T")[0];
    (document.getElementById("tour-type") as HTMLSelectElement).value =
      tourType;
    (document.getElementById("tour-tags-info") as HTMLSpanElement).innerText =
      this.configStore.Options.tags.tagHelpText;

    document.getElementById("tour-title").onkeyup = this.checkTourTitle;
    document.getElementById("tour-description").onkeyup =
      this.checkTourDescription;
    document.getElementById("tour-activeon").onchange = this.checkTourActiveon;
    document.getElementById("tour-expireson").onchange =
      this.checkTourExpireson;
    document.getElementById("tour-tags").onkeyup = this.checkTourTags;
    document.getElementById("cover-page-btn").onclick =
      this.createCoverPageModal;
    ...
    document.getElementById("add-tour-modal-close").onclick =
      this.closeAddTourModal;
    document.getElementById("add-tour-modal-cancel-btn").onclick =
      this.closeAddTourModal;
  ...
  private checkTourTitle = () => {
    const tourTitleTextArea: HTMLTextAreaElement = document.getElementById(
      "tour-title"
    ) as HTMLTextAreaElement;
    let value = tourTitleTextArea.value;
    document.getElementById("tour-title-error").style.display =
      value === "" ? "block" : "none";
    document.getElementById("tour-title-character-limit").innerText =
      74 - value.length + " characters remaining.";
  };

  /// Validates Tour Description
  private checkTourDescription = () => {
    const tourDescriptionTextArea: HTMLTextAreaElement =
      document.getElementById("tour-description") as HTMLTextAreaElement;
    let value = tourDescriptionTextArea.value;
    document.getElementById("tour-description-error").style.display =
      value === "" ? "block" : "none";
  ...
  private checkTourActiveon = () => {
    const tourActiveOnInputElement: HTMLInputElement = document.getElementById(
      "tour-activeon"
    ) as HTMLInputElement;
  ...
  private checkTourExpireson = () => {
    const tourExpiresOnInputElement: HTMLInputElement = document.getElementById(
      "tour-expireson"
    ) as HTMLInputElement;
  ...
  private checkTourTags = () => {
    const tourTagsTextArea: HTMLTextAreaElement = document.getElementById(
      "tour-tags"
    ) as HTMLTextAreaElement;
    let value = tourTagsTextArea.value;
    document.getElementById("tour-tags-character-limit").innerText =
  ...
  private saveTour = async () => {
    ...
    const saveTourModalButton: HTMLButtonElement = document.getElementById(
      "save-tour-modal-btn"
    ) as HTMLButtonElement;
    saveTourModalButton.disabled = true;

    let tourTitleBox: HTMLTextAreaElement = document.getElementById(
      "tour-title"
    ) as HTMLTextAreaElement;
    let tourDescriptionBox: HTMLTextAreaElement = document.getElementById(
      "tour-description"
    ) as HTMLTextAreaElement;
    let tourActiveOnBox: HTMLInputElement = document.getElementById(
      "tour-activeon"
    ) as HTMLInputElement;
    let tourExpiresOnBox: HTMLInputElement = document.getElementById(
      "tour-expireson"
    ) as HTMLInputElement;
    let isTourAutoPlayEnabledFlag: HTMLInputElement = document.getElementById(
      "isAutoPlayEnabled"
    ) as HTMLInputElement;
    let tags: HTMLTextAreaElement = document.getElementById(
      "tour-tags"
    ) as HTMLTextAreaElement;
    ...
    if (this.configStore.Options.appInfo) {
      for (let [key, value] of Array.from(
        this.configStore.Options.appInfo.entries()
      )) {
    ...
    if (this.tour == null || this.tour.id == null) {
      ...
      try {
        ...
        if (this.configStore.Options.navigator.callbackOnTourSavedFailed) {
          this.configStore.Options.navigator.callbackOnTourSavedFailed(
            err as string
          );
        }
      }
  ...
  private tourTypeChanged = () => {
    let tourtypeselect = document.getElementById(
      "tour-type"
    ) as HTMLSelectElement;
    let autoPlayCheckbox = document.getElementById(
      "isAutoPlayEnabled"
    ) as HTMLInputElement;
    ...
    if (tourtype.toLowerCase() == TourTypeEnum.Announcement.toLowerCase()) {
      ...
      tourtype.toLowerCase() == TourTypeEnum.Beacon.toLocaleLowerCase()
    ) {
  ...
  private validateandSetFocus = (controlsList: string[]) => {
    for (let index = 0; index < controlsList.length - 1; index++) {
      if (
        document.getElementById(controlsList[index] + "-error").style
          .display === "block"
      ) {
  ...
  /// Loads Tour Dialog
  private loadTour = () => {
    (document.getElementById("tour-title") as HTMLTextAreaElement).value =
      this.tour.title;
    (document.getElementById("tour-description") as HTMLTextAreaElement).value =
      this.tour.description;
    (document.getElementById("tour-tags") as HTMLTextAreaElement).value =
      this.tour.tags;
    (document.getElementById("tour-activeon") as HTMLInputElement).value =
      this.tour.activeon.split("T")[0];
    (document.getElementById("tour-expireson") as HTMLInputElement).value =
      this.tour.expireson.split("T")[0];
    (document.getElementById("isAutoPlayEnabled") as HTMLInputElement).checked =
      this.tour.isautoplayenabled;
    (document.getElementById("tour-type") as HTMLSelectElement).value =
      this.tour.tourtype;
    ...
    if (this.tourCoverPageContent !== "" && this.tourCoverPageLocation !== "") {
      ...
      addCoverPageButton.innerHTML =
        '<i class="pagetour__icon icon-pencil icon-inline"></i>Edit Cover Page';
  ...
  private previewTour = () => {
    if (
      ...
      tour.title = (
        document.getElementById("tour-title") as HTMLTextAreaElement
      ).value;
      tour.description = (
        document.getElementById("tour-description") as HTMLTextAreaElement
      ).value;
      tour.tourtype = (
        document.getElementById("tour-type") as HTMLSelectElement
      ).value;
      ...
      if (
        tour.tourtype.toLowerCase() == TourTypeEnum.Announcement.toLowerCase()
      )
        this.pageTourPlay.runAnnouncement(
          tour,
          RunTourAction.Preview,
          0,
          this.addTourDialog
        );
      else if (
        tour.tourtype.toLowerCase() == TourTypeEnum.Beacon.toLocaleLowerCase()
      )
        this.pageTourPlay.runSmartTip(
          tour,
          RunTourAction.Preview,
          0,
          this.addTourDialog
  ...
  private populateSteps = () => {
    ...
    for (let i = 0; i < this.stepList.length; i++) {
      ...
      let tourType =
        this.tour && this.tour.tourtype
          ? this.tour.tourtype
          : (document.getElementById("tour-type") as HTMLSelectElement).value;
      if (tourType.toLowerCase() == TourTypeEnum.Announcement.toLowerCase()) {
        document.getElementById("step-tourtype-header").innerText =
          "Header Text";
        document.getElementById("step-tourtype-header").style.width = "250px";
        document.getElementById("step-announcement-image-url").style.display =
          "inline";
    ...
    if (this.stepList.length > 0) {
      (
        document.getElementById("save-tour-modal-btn") as HTMLButtonElement
      ).disabled = false;
      (
        document.getElementById("preview-tour-step-btn") as HTMLButtonElement
      ).disabled = false;
    } else {
      document
        .getElementById("save-tour-modal-btn")
        .setAttribute("disabled", "disabled");
      document
        .getElementById("preview-tour-step-btn")
        .setAttribute("disabled", "disabled");
    }

    if (this.tourCoverPageLocation || this.stepList.length > 0) {
      (document.getElementById("tour-type") as HTMLSelectElement).disabled =
        true;
    } else {
      (document.getElementById("tour-type") as HTMLSelectElement).disabled =
        false;
  ...
  private getButton = (buttonType: any, index: number) => {
    ...
    switch (buttonType) {
      case "expander":
        ...
        button.setAttribute("aria-label", "Row Collapse button");
  ...
  private getStepMessageElement = (index: number) => {
    ...
    msgElement.appendChild(
      document.createTextNode(this.stepList[index].message)
    );
  ...
  private deleteStep = (index: any) => {
    ...
    let nextFocusableElement =
      this.getNextFocusableElementAfterDeleteStep(index);
  ...
  private getNextFocusableElementAfterDeleteStep(index: any): HTMLElement {
    ...
    switch (this.stepList.length) {
      ...
      case 0:
        nextFocusableElement = document.getElementById(
          "add-tour-modal-cancel-btn"
        );
        break;
      ...
      default:
        ...
        // if any step other than the last was deleted
          ...
          let moveUp: HTMLButtonElement = document.getElementById(
            `button-moveup_${index}`
          ) as HTMLButtonElement;
          let moveDown = document.getElementById(
            `button-movedown_${index}`
          ) as HTMLButtonElement;
  ...
  private editTourStep() {
    this.createChooseElementModal();
    document.getElementById("choose-element-title").innerText =
      "Edit Step - Choose an element";
    document
      .getElementById("close-btn")
      .setAttribute("aria-label", "Close Edit step dialog");
    document
      .getElementById("cancel-choose-element-btn")
      .setAttribute("aria-label", "cancel and Close Edit Step dialog");
    event.preventDefault();
    event.stopPropagation();
    ...
    if (
      ...
      this.stepList.length > this.editStepIndex
    ) {
  ...
  private editAnnouncementStep() {
    this.addAnnouncementPage();
    document.getElementById("announcement-page-modal-title").innerText =
      "Edit Announcement Step";
    document
      .getElementById("announcement-page-close-btn")
      .setAttribute("aria-label", "Close Edit Announcement step dialog");
    document
      .getElementById("cancel-announcement-page-btn")
      .setAttribute(
        "aria-label",
        "Cancel and Close Edit Announcement Step dialog"
      );
    event.preventDefault();
    event.stopPropagation();
    if (
      ...
      const editingStep = this.stepList[this.editStepIndex];
      document.getElementById("input-announcement-header-text").innerText =
        editingStep.headerText;
      document.getElementById("announcementboxdescription").innerHTML =
        document.getElementById("anno-message-editor").innerHTML =
          editingStep.message;
      this.announcementHeaderChange();
      if (editingStep.mediaUrl)
        document.getElementById("input-announcement-image-video").innerText =
          editingStep.mediaUrl;
      this.livePreviewMediaHeader();
      if (editingStep.transcript)
        document.getElementById(
          "transcript-message-for-announcement"
        ).innerText = editingStep.transcript;
    }
  }

  private ignoreStepIfSetup = (tourstep: Step) => {
    let ignoreNextIfNextStepElementFound: HTMLInputElement =
      document.getElementById(
  ...
  private toggleStepMessage = (index: number) => {
    ...
    if (iconExpander.classList.contains("icon-drop")) {
      ...
      document
        .getElementById("step-message-element_" + index)
        .setAttribute("class", "message-desc-expanded");
      document
        .getElementById("icon-drop_" + index)
        .setAttribute("class", "pagetour__icon icon-drop-up");
    } else {
      document
        .getElementById("step-message-element_" + index)
        .setAttribute("class", "message-desc");
      document
        .getElementById("icon-drop_" + index)
  ...
  private createChooseElementModal = () => {
    ...
    let cancelChooseElement = document.getElementById(
      "cancel-choose-element-btn"
    );
    cancelChooseElement.onclick = this.closeChooseElementModal;
    let tourType = (document.getElementById("tour-type") as HTMLSelectElement)
      .value;
    let nextElement = document.getElementById(
      "select-element-next-btn"
    ) as HTMLButtonElement;

    const chosenElementTextArea: HTMLTextAreaElement = document.getElementById(
      "chosen-element"
    ) as HTMLTextAreaElement;
    ...
    nextElement.addEventListener(
      "click",
      tourType && tourType.toLowerCase() === TourTypeEnum.Beacon.toLowerCase()
        ? this.createSmartTipBox
        : this.createRecordBox
    );
  ...
  private toggleChooseElement = (state: any, editingStep: any) => {
    let chooseElement: HTMLButtonElement = document.getElementById(
      "choose-element-btn"
    ) as HTMLButtonElement;
  ...
  private showHideIgnoreKeyElement = (show: boolean, editingStep: any) => {
    ...
    let ignoreKeyCheckboxElementLbl = document.getElementById(
      "ignore-element-key-chkbox-lbl"
    );
    if (show && show === true) {
      ...
      if (this.lastSelectedElement) {
        if (
          editingStep &&
          editingStep.ignoreelementkey &&
          editingStep.ignoreelementkey === true
        ) {
  ...
  private disablePageInspector = (abort: any) => {
    ...
    if (inspectorInline && inspectorInline.length > 0) {
      ...
      if (abort) {
        if (this.lastSelectedElement) {
          this.lastSelectedElement.style.outline =
    ...
    inspector.addEventListener("click", (event) => {
      if (this.lastSelectedElement) {
        if (event.altKey) {
          ...
          if (this.lastSelectedElement != null) {
            this.toggleChooseElement(this.chooseState.Chosen, {
              selector: self.lastSelectedElementSelector,
            });
          }
        }
      }
  ...
  private onBodyMouseMove = (event: HTMLElementEventMap["mousemove"]) => {
    ...
    if (el === document.body) {
      ...
      while (el && el.shadowRoot != null && count < 10) {
        ...
        el = el.shadowRoot.elementFromPoint(
          event.clientX,
          event.clientY
        ) as HTMLElement;
        //console.log(el);
        this.lastSelectedElementSelector +=
          " " + getCssSelector(el, { root: shadowRoot });
        count++;
      }
  ...
  private createCoverPageModal = () => {
    ...
    if (!coverPageModal) {
      ...
      let coverPageTextArea = document.getElementById(
        "cover-page-content"
      ) as HTMLTextAreaElement;
    ...
    let coverPagePositionElement = document.getElementById(
      "cover-location-select"
    ) as HTMLSelectElement;
    ...
    const cancelChooseElement = document.getElementById(
      "cancel-cover-page-btn"
    );
  ...
  private addAnnouncementPage = () => {
    ...
    announcementPageModal.style.display = "block";
    (
      document.getElementById("videoHeaderContainer") as HTMLVideoElement
    ).style.display = "none";
    this.setDefaultImage();
    this.initiateCkEditor();

    let announcementPageForm = document.getElementById(
    ...
    document.getElementById("record-announcement-page-btn").onclick =
      this.recordAnnouncement;
    document.getElementById("stop-record-announcement-page-btn").onclick =
      this.stopAnnouncementRecording;

    document.getElementById("input-announcement-header-text").onkeyup =
      this.announcementHeaderChange;
    document.getElementById("input-announcement-image-video").onkeyup =
      this.livePreviewMediaHeader;
    document.getElementById("input-announcement-image-video").onblur =
      this.setDefaultImage;

    document.getElementById("cancel-announcement-page-btn").onclick =
      this.closeAnnouncementPageModal;
    document.getElementById("save-announcement-page-btn").onclick =
      this.saveAnnouncementPage;

    if (!this.configStore.Options.enableTranscript)
      document.getElementById("transcript-announcement-area").style.display =
        "none";
  };

  private async initiateCkEditor() {
    ClassicEditor.create(document.getElementById("anno-message-editor"), {
      ...
      .then((editor: any) => {
        ...
        editor.editing.view.document.on("keydown", (event: any, data: any) => {
          this.checkKeyboardBlocking(
            $(editor.getData()).text().length,
            event,
            data
          );
        });
  ...
  private setDefaultImage() {
    let mediaUrl = document.getElementById(
      "input-announcement-image-video"
    ) as HTMLTextAreaElement;
    if (mediaUrl.value == "") {
      document.getElementById("anno-media-error").style.display = "none";
      let mediaDiv = document.getElementById(
        "imgHeaderContainer"
      ) as HTMLImageElement;
      // Todo: make this url injection dynamic
      if (
        this.configStore !== null &&
        this.configStore.Options.announcementDefaultImage
      )
        mediaDiv.src = this.configStore.Options.announcementDefaultImage;
      else mediaDiv.alt = "Default Image not configured";
    }
  }
  ...
  private announcementHeaderChange() {
    let headerValue = (
      document.getElementById(
        "input-announcement-header-text"
      ) as HTMLInputElement
    ).value;
    if (headerValue == "") {
      ...
      document.getElementById("announcementboxtitle-preview").innerText =
        headerValue;
    }
  ...
  private livePreviewMediaHeader() {
    ...
    if (!isValidUrl && mediaUrl.value !== "") {
      ...
      document.getElementById("anno-media-error").style.display = "none";
      let imgHeaderContainer = document.getElementById(
        "imgHeaderContainer"
      ) as HTMLImageElement;
      let videoHeaderContainer = document.getElementById(
  ...
  private checkCoverPagePosition = () => {
    const coverPagePositionElement = document.getElementById(
      "cover-location-select"
    ) as HTMLSelectElement;
    ...
    const value =
      coverPagePositionElement.options[coverPagePositionElement.selectedIndex]
        .value;
  ...
  private updateModalTitle = () => {
    if (this.tourCoverPageContent !== "" && this.tourCoverPageLocation !== "") {
      document.getElementById("cover-page-modal-title").innerText =
        "Edit Cover Page";
    }
  };
  ...
  private checkCoverPageBodyText = () => {
    ...
    if (coverPageTextArea.value === "") {
      document.getElementById("coverpage-bodytext-error").style.display =
        "block";
    } else {
      document.getElementById("coverpage-bodytext-error").style.display =
        "none";
  ...
  private saveCoverPage = () => {
    let coverPageElement = document.getElementById(
      "cover-location-select"
    ) as HTMLSelectElement;
    let positionSelectedElement =
      coverPageElement.options[coverPageElement.selectedIndex];
  ...
  private closeAnnouncementPageModal = () => {
    let announcementPageModal = document.getElementById(
  ...
  private saveAnnouncementPage = () => {
    let announcementHeader = document.getElementById(
      "input-announcement-header-text"
    ) as HTMLTextAreaElement;
  ...
  private StopTranscriptGeneration(type: string) {
    let transcriptBtnIcon = document.getElementById(
      "record-" + type + "-page-btn"
    ) as HTMLButtonElement;
    let stopTranscriptBtnIcon = document.getElementById(
      "stop-record-" + type + "-page-btn"
    ) as HTMLButtonElement;
  ...
  private GenerateTranscript(type: string) {
    ...
    self.recognition.onend = function () {
      ...
      if (!self.hasUserStoppedRecording) self.recognition.start();
      else self.recognition.stop();
    };
    ...
    self.recognition.onresult = function (event: any) {
      ...
      else transcriptDiv.value = event.results[0][0].transcript;
    };
  ...
  private getAnnouncementPageDetails = () => {
    let headerElement = document.getElementById(
      "input-announcement-header-text"
    ) as HTMLTextAreaElement;
    let mediaUrlElement = document.getElementById(
      "input-announcement-image-video"
    ) as HTMLTextAreaElement;
    let transcriptElement = document.getElementById(
      "transcript-message-for-announcement"
    ) as HTMLTextAreaElement;
    let pageContext = this.getPageContext();
    if (this.tour == null) this.tour = {};
    ...
    if (transcriptElement) newStep.transcript = transcriptElement.value;
  ...
  private createRecordBox = (event: Event) => {
    ...
    if (!stepDetailModal) {
      ...
      let delayBeforeStepSlider: HTMLInputElement = document.getElementById(
        "delayBeforeStepSlider"
      ) as HTMLInputElement;
      let delayBeforeStepValue: HTMLInputElement = document.getElementById(
        "delay-for-step"
      ) as HTMLInputElement;
      ...
      if (this.editStepIndex !== -1) {
        document.getElementById("step-detail-modal-title").innerText =
          "Edit Step - Details";
    ...
    if (!this.configStore.Options.enableTranscript)
      document.getElementById("transcript-pagetour-area").style.display =
        "none";
  ...
  private createSmartTipBox = (event: Event) => {
    ...
    if (!stepDetailModal) {
      ...
      document.getElementById("message-for-smart-tip").onkeyup =
        this.checkSmartTipMessage;

      /// Loads Step details in a Record box during Edit.
      if (this.editStepIndex !== -1) {
        document.getElementById("tip-detail-modal-title").innerText =
          "Edit Smart Tip - Details";
  ...
  private checkSmartTipMessage() {
    let messageValue = (
      document.getElementById("message-for-smart-tip") as HTMLInputElement
    ).value;
    if (messageValue == "") {
      document.getElementById("message-for-smart-tip-error").style.display =
        "contents";
    } else {
      document.getElementById("message-for-smart-tip-error").style.display =
        "none";
  ...
  private checkEventType = () => {
    let eventTypeElement: HTMLSelectElement = document.getElementById(
      "event-type-select"
    ) as HTMLSelectElement;
    ...
    let hintText = null;
    if (value === "") {
      document.getElementById("event-type-select-error").style.display =
        "block";
  ...
  private checkPositionSelect = () => {
    let positionSelectElement: HTMLSelectElement = document.getElementById(
      "position-select"
    ) as HTMLSelectElement;
    document.getElementById("position-select-error").style.display =
      positionSelectElement.options[positionSelectElement.selectedIndex]
        .value === ""
        ? "block"
        : "none";
  };
  ...
  private checkMessageForStep = (tourtype: string) => {
    let messageForStepErrorElement: HTMLTextAreaElement =
      document.getElementById(`message-for-${tourtype}`) as HTMLTextAreaElement;
  ...
  private checkValueForStep = () => {
    let valueForStepElement: HTMLTextAreaElement = document.getElementById(
      "value-for-step"
    ) as HTMLTextAreaElement;
    document.getElementById("value-for-step-error").style.display =
      (valueForStepElement.disabled === false &&
        valueForStepElement.value === "") === true
        ? "block"
        : "none";
  };
  ...
  private checkDelayValue = () => {
    let delayInputSlider: HTMLInputElement = document.getElementById(
      "delayBeforeStepSlider"
    ) as HTMLInputElement;
    let delayInputBox: HTMLInputElement = document.getElementById(
      "delay-for-step"
    ) as HTMLInputElement;
  ...
  private toggleValueDisplay = () => {
    let eventTypeSelectBtn: HTMLSelectElement = document.getElementById(
      "event-type-select"
    ) as HTMLSelectElement;
    let valueForStepArea: HTMLTextAreaElement = document.getElementById(
      "value-for-step"
    ) as HTMLTextAreaElement;
    let valueForStepAreaLabel: HTMLElement = document.getElementById(
      "value-for-step-label"
    );
  ...
  private getStepDetails = () => {
    ...
    let messageForStepElement: HTMLTextAreaElement = document.getElementById(
      "message-for-step"
    ) as HTMLTextAreaElement;
    let errorMessageForStepElement: HTMLTextAreaElement =
      document.getElementById("error-message-for-step") as HTMLTextAreaElement;
    let valueForStepElement: HTMLTextAreaElement = document.getElementById(
      "value-for-step"
    ) as HTMLTextAreaElement;
    let delayBeforeStepSlider: HTMLInputElement = document.getElementById(
    ...
    let chosenElement: HTMLTextAreaElement = document.getElementById(
      "chosen-element"
    ) as HTMLTextAreaElement;

    let eventType =
      eventTypeElement.options[eventTypeElement.selectedIndex].value;
    let position =
      positionSelectElement.options[positionSelectElement.selectedIndex].value;
    ...
    let delayBefore: number = parseInt(
      (document.getElementById("delay-for-step") as HTMLInputElement).value,
      10
    );
    ...
    if (
      ignoreNextIfNextStepElementFound &&
      ignoreNextIfNextStepElementFound.checked
    ) {
  ...
  private resetStepDetailsModal = () => {
    ...
    if (stepDetailModal) {
      (
        document.getElementById("event-type-select") as HTMLSelectElement
      ).value = "click";
      (document.getElementById("position-select") as HTMLSelectElement).value =
        "right";
      (
        document.getElementById("message-for-step") as HTMLTextAreaElement
      ).value = "";
      (
        document.getElementById("error-message-for-step") as HTMLTextAreaElement
      ).value = "";
  ...
  private populateStepDetails = () => {
    ...
    (document.getElementById("event-type-select") as HTMLSelectElement).value =
      step.type;
    (document.getElementById("position-select") as HTMLSelectElement).value =
      step.position;
  ...
  private populateTipDetails = () => {
    let step = this.stepList[this.editStepIndex];
    (
      document.getElementById("message-for-smart-tip") as HTMLTextAreaElement
    ).value = step.message;
    (document.getElementById("position-select") as HTMLSelectElement).value =
      step.position;
